...
def read_BronzeTrafficTable(catalog, environment):
    print('Reading the Bronze Table Data : ',end='')
    df_bronzeTraffic = (spark.readStream
                    .table(f"`{catalog}`.`bronze`.raw_traffic_{environment}")
                    )
    print(f'Reading {catalog}.bronze.raw_traffic_{environment} Success!')
...
def write_Traffic_SilverTable(StreamingDF,catalog,environment):
    ...
    write_StreamSilver = (StreamingDF.writeStream
                ...
                .toTable(f"`{catalog}`.`silver`.`silver_traffic_{environment}`"))
    
    write_StreamSilver.awaitTermination()
    print(f'Writing `{catalog}`.`silver`.`silver_traffic_{environment}` Success!')

# COMMAND ----------

## Reading the bronze traffic data

df_trafficdata = read_BronzeTrafficTable(catalog, env)
...
write_Traffic_SilverTable(df_final, catalog, env)

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC SELECT COUNT(*) FROM `hive_metastore`.`silver`.`silver_traffic_dev`

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC SELECT * FROM `hive_metastore`.`silver`.`silver_traffic_dev`
