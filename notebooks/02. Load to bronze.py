...
def write_Traffic_Data(StreamingDF,catalog,environment):
    print(f'Writing data to {catalog}_catalog raw_traffic_{environment} table', end='' )
    write_Stream = (StreamingDF.writeStream
                    ...
                    .toTable(f"`{catalog}`.`bronze`.`raw_traffic_{environment}`"))
...
def write_Road_Data(StreamingDF,catalog,environment):
    print(f'Writing data to {catalog}_catalog raw_roads_{environment} table', end='' )
    write_Data = (StreamingDF.writeStream
                    ...
                    .trigger(availableNow=True)
                    .toTable(f"`{catalog}`.`bronze`.`raw_roads_{environment}`"))
...
write_Traffic_Data(read_Df,catalog,env)

## Writing the raw_roads's data from landing to Bronze
write_Road_Data(read_roads,catalog,env)
...
display(spark.sql(f"""SELECT * FROM hive_metastore.bronze.raw_traffic ORDER BY Extract_Time DESC"""))

# COMMAND ----------

display(spark.sql(f"""SELECT COUNT(*) FROM hive_metastore.bronze.raw_traffic_{env}"""))

# COMMAND ----------

display(spark.sql(f"""SELECT * FROM hive_metastore.bronze.raw_roads_{env}"""))
